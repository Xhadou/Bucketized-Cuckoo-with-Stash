...
\begin{itemize}
    \item \emph{Breadth.} We implement and benchmark nine hash table variants (four cuckoo-family and five baselines) behind a unified Java interface, along with three pluggable hash functions (MurmurHash3, xxHash32, FNV-1a) on the JVM. This produces a head-to-head comparison at a scale not, to our knowledge, previously published in a single study.
...
We implemented nine hash table variants behind a unified Java interface \texttt{CuckooHashTable<K,V>} with six methods: \texttt{get}, \texttt{put}, \texttt{remove}, \texttt{size}, \texttt{loadFactor}, and \texttt{getStats}. We describe each technique below, grouped by family.
...
\subsection{Hash Functions}
We implement three hash functions behind a functional interface \texttt{HashFamily} with signature \texttt{int hash(int keyHash, int seed)}:
...
\begin{itemize}
    ...
    \item \textsc{RealDataBenchmark}: all nine variants on $100{,}000$ unique Wikipedia page titles (string keys) extracted from a pageview dump~\cite{wikipedia_pageviews}.
\end{itemize}
...
Beyond its empirical contributions, our implementation is available as a pedagogical resource. Each of the nine hash table variants is implemented in under $400$ lines of idiomatic Java~25, with a shared interface, a full unit test suite ($115$ tests), and a JMH harness that produces publication-quality benchmark data in under two hours of wall time. Students or implementers wishing to understand these schemes empirically---rather than only through their theoretical analyses---have, we hope, a useful starting point.
