...
\begin{itemize}
    ...
    \item \emph{Falsifiable hypotheses.} We formulate five explicit hypotheses (H1--H5) with falsification criteria tied to specific measurable thresholds, test each empirically, and report the falsification of H4 as an honest finding: \emph{bucketization is a memory-efficiency optimization, not a speed optimization.} This is, we believe, a useful corrective to the narrative common in the cuckoo-hashing literature that positions bucketization as a universal improvement.
\end{itemize}
...
\emph{xxHash32} uses four prime-like constants and a single-block ingest followed by a three-multiply finalization avalanche. In C with SIMD it achieves $\sim 31$\,GB/s, but in the JVM without vectorization its advantage over MurmurHash3 shrinks to within measurement noise.
...
The practical performance of a hash table depends as much on its memory layout as on its asymptotic complexity. A single L3 cache miss on modern Intel or Apple Silicon hardware costs $40$--$80$ nanoseconds; a main-memory DRAM access costs $100$--$200$ nanoseconds. For a typical integer-keyed hash table, these latencies dwarf the $5$--$20$ nanoseconds of hash function computation. Consequently, the number of \emph{cache misses} per operation is often the dominant predictor of throughput.
...
\begin{itemize}
    ...
    \item \textsc{HashFunctionBenchmark}: bucketized ($B=4$) throughput for MurmurHash3, xxHash32, and FNV-1a.
...
This agreement serves as a sanity check on our implementation and a direct empirical validation of the $d$-ary theory---part of the reason we spent significant effort implementing the $d$-ary variant even though it is not central to the bucketization/stash narrative.
...
\textbf{Verdict: H4 Falsified for bucketized cuckoo.} The honest finding: bucketization buys \emph{memory efficiency}, not lookup speed. A practitioner optimizing for lookup throughput should prefer standard cuckoo or hopscotch over the bucketized variant.
...
At $n=100{,}000$, MurmurHash3, xxHash32, and FNV-1a produce statistically indistinguishable throughput on bucketized cuckoo (the three values $82.2$, $84.5$, and $85.6$\,ops/s for insertion are within confidence intervals). At $n=500{,}000$, positive lookup throughput diverges: MurmurHash3 gives $15.1$\,ops/s, xxHash32 $14.4$\,ops/s, but \emph{FNV-1a collapses to $7.9$\,ops/s}---roughly half the throughput of the other two, despite the same algorithmic implementation and the same table parameters ($B=4$).

\begin{table}[h]
\centering
\caption{Hash function throughput (ops/s) on bucketized ($B=4$) cuckoo.}
\label{tab:hashfunc}
\begin{tabular}{@{}lrr@{}}
\toprule
Hash family & $n=100\mathrm{K}$ & $n=500\mathrm{K}$ \\
\midrule
MurmurHash3 (pos. lookup) & $168.8 \pm 5.1$ & $15.1 \pm 0.7$ \\
xxHash32    (pos. lookup) & $166.5 \pm 4.4$ & $14.4 \pm 0.9$ \\
FNV-1a       (pos. lookup) & $183.3 \pm 5.0$ & $\mathbf{7.9 \pm 0.1}$ \\
\bottomrule
\end{tabular}
\end{table}

The divergence is subtle but consequential: FNV-1a's weaker avalanche passes SMHasher's smaller-bucket tests but causes a measurable rise in average displacement chain length once the table is large enough that cache pressure dominates. Our \textsc{DirectAnalysis} harness independently confirms nearly equal average chain lengths ($0.15$) across all three hash functions at $n=20{,}000$, indicating the effect only manifests at larger scales. A lesson for the community: hash function quality is often dismissed as irrelevant for cuckoo integer keys. At small scales this is true, but scale-dependent effects emerge once cache pressure interacts with uneven bucket loads. When benchmarking hash-table variants, it is not sufficient to test a single function at a single scale.
...
\subsection{Deletion Throughput: Addressing the Professor's Concern}

Table~\ref{tab:delete} shows deletion throughput for $100{,}000$ elements. Standard cuckoo matches chaining ($124$ vs.\ $123$\,ops/s), and outperforms every probing baseline (linear $83$, quadratic $98$) substantially. This is the empirical signature of the theoretical claim: \emph{cuckoo hashing is the only open-addressing scheme with tombstone-free $O(1)$ deletion}, a point we highlighted in Section~\ref{sec:techniques}.
...
Motivated by reviewer feedback requesting coverage of theoretically-sound hash families, we added two further comparison points: the Carter-Wegman universal hash family~\cite{carter1979universal} as a fourth \texttt{HashFamily} implementation, and Fredman-Kom\-l\'os-Szemer\'edi perfect hashing~\cite{fks1984perfect} as a static lookup-only baseline.

\paragraph{Carter-Wegman universal vs engineered mixers.}
Running the hash function sensitivity analysis (Section~\ref{sec:hypotheses}, H5) with the fourth family yields the result in Figure~\ref{fig:hash-sensitivity-updated}: across five trials of $50{,}000$ insertions into bucketized cuckoo ($B{=}4$), Carter-Wegman produces an average displacement chain length of $0.152 \pm 0.002$, statistically indistinguishable from MurmurHash3 ($0.149 \pm 0.004$), xxHash32 ($0.151 \pm 0.002$), and FNV-1a ($0.153 \pm 0.005$). All four families achieve the same maximum load factor ($\sim 0.900$) with zero rehashes. The theoretical guarantee does not translate into measurably different \emph{algorithmic} behavior on these workloads---consistent with the long-standing folklore that any hash function ``good enough'' to pass modest avalanche tests is effectively interchangeable for cuckoo-family use. Our contribution here is empirical confirmation on a fourth, theoretically-provable family.

\paragraph{FKS perfect hashing vs dynamic variants.}
Perfect hashing is a static dictionary: all keys must be known at construction. This makes a side-by-side lookup-only comparison the only meaningful comparison. Table~\ref{tab:perfect} reports positive-lookup throughput on $50{,}000$ integer keys; construction time is also shown for reference.

\begin{table}[h]
\centering
\caption{Lookup throughput (positive lookup, $n=50{,}000$) comparing FKS perfect hashing against dynamic variants. Build time refers to construction from a known key set (perfect) or serial \texttt{put()} of all keys (dynamic).}
\label{tab:perfect}
\small
\begin{tabular}{@{}lrr@{}}
\toprule
Variant            & Lookup (ops/s) & Build (ms) \\
\midrule
Standard cuckoo    & $33{,}843{,}044$ & $1$  \\
Perfect (FKS)      & $32{,}408{,}398$ & $16$ \\
Quadratic probing  & $30{,}659{,}046$ & $5$  \\
Hopscotch          & $29{,}214{,}126$ & $4$  \\
Robin Hood         & $21{,}564{,}094$ & $15$ \\
Linear probing     & $11{,}681{,}735$ & $5$  \\
Bucketized ($B=4$) & $7{,}510{,}373$  & $10$ \\
\bottomrule
\end{tabular}
\end{table}

FKS perfect hashing achieves $32.4$\,Mops/s---essentially tied with standard cuckoo ($33.8$\,Mops/s, a $4\%$ gap within measurement noise) and ahead of every open-addressing baseline. This result has two interesting implications. First, the theoretical ``zero collisions, at-most-two-probes'' guarantee of FKS does not translate into a throughput advantage over standard cuckoo at this scale, because standard cuckoo's two-probe lookup already achieves the same cache footprint and the difference lies only in whether the second probe is deterministic (FKS) or speculative (cuckoo). Second, FKS dominates bucketized cuckoo by $4.3\times$ on lookups precisely because it avoids the linear bucket scan that dominates bucketized runtime. Construction costs $16$\,ms versus $1$\,ms for the dynamic equivalent---about an order of magnitude slower, reflecting the FKS retry loops---but this is a one-time cost amortized across all subsequent lookups. The practical takeaway: \emph{for read-only dictionaries of known keys, FKS is fully competitive with the fastest dynamic schemes and strictly faster than bucketized variants}; for dynamic workloads it is simply not applicable.
...
Promising directions include: (1) porting this benchmark suite to a Linux/x86 platform and using \texttt{perf} to perform cache-miss decomposition; (2) adding a concurrent variant and comparing against libcuckoo's lock-striped bucketized cuckoo; (3) evaluating all variants on CAIDA network traces as a stress test for adversarial access patterns; (4) adding cryptographic hash functions such as SipHash to study hash-DoS resistance; and (5) exploring whether the surprising strength of standard cuckoo at high load holds when $B$-way bucketized structures are more carefully cache-aligned.
...
\subsection{Teaching Value}
Beyond its empirical contributions, our implementation is available as a pedagogical resource. Each of the ten hash table variants is implemented in under $400$ lines of idiomatic Java~25, with a shared interface, a full unit test suite ($126$ tests), and a JMH harness that produces publication-quality benchmark data in under two hours of wall time. Students or implementers wishing to understand these schemes empirically---rather than only through their theoretical analyses---have, we hope, a useful starting point.

\subsection{Closing Thought}
The cuckoo-family literature is dense with theoretical elegance---load factor thresholds, displacement chain analyses, stash overflow bounds---yet comparatively light on rigorous cross-variant empirical evaluation with real data. Our work offers one such evaluation, together with the observation that the most popular ``improvements'' (bucketization, stash) are not universally improvements in practice, but rather specific trade-offs against specific constraints. We hope this motivates both more careful benchmarking methodology in the hash-table literature and a more nuanced reading of the cuckoo-family design space. The best variant is not universally the bucketized one; it depends on which resource---memory, lookup latency, insertion throughput, or deletion cost---is the binding constraint in the target application.
